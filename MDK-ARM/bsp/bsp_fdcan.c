	   ...
	   FDCAN_IT_RX_FIFO0_FULL == RxFifo0ITs)
    {
    	do
    	{
    		...
    		if(hfdcan1_RX.RxFrameType == FDCAN_REMOTE_FRAME)
				...
				memcpy(CANopen_Slave.RxMSG.data,(uint8_t *)fdcan1.rx_MSG.buffer,fdcan1.rx_MSG.len);  /* 数据段 */
				CANopen_Slave.RxMSG.cob_id = fdcan1.rx_MSG.id;                              				 /* CAN-ID */
				CANopen_Slave.RxMSG.rtr = fdcan1.rx_MSG.rtr;																			   /* 帧类型 */
				CANopen_Slave.RxMSG.len = fdcan1.rx_MSG.len;																		     /* 帧长度 */

				canDispatch(CANopen_Slave.object_DIC,&CANopen_Slave.RxMSG);  /* 将接收到的CAN报文提供给Canfestival（CANopen协议）*/
